...
# This is not the same as the Lens shown in the Zemax program window
# (Lens Data Editor).  The Lens can be copied in and out of the Lens
# Data Editor by the commands PushLens and GetRefresh, respectively.
...
def returns_error_status(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if error:
            raise ZemaxServerError(
                "%s signalled an error: %s" % (func.__name__, str(error)))
    return wrapper
...
class Connection:
    ...
    Methods on this class closely resemble the commands (or "data
    items") available to Zemax extensions.  Chapter 23 of the Zemax
    manual "Zemax Extensions" explains how the commands work.
    """
    ...
    def ExportCAD(self, filename, filetype, numspline=0, first=0,
                  last=-1, rayslayer=0, lenslayer=1,
                  exportdummy=False, usesolids=True, raypattern=0,
                  numrays=10, wave=0, field=0, deletevignetted=True,
                  dummythick=1.0, split=False, scatter=False,
                  usepol=False, config=0):
        """
        Arguments:
            ...
            raypattern: {0:XY, 1:X, 2:Y, 3:ring, 4:list, 5:none, 6:grid,
                         7:solid beam}
        ...
        cmd = ("ExportCAD,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.20E,"
               "%d,%d,%d,%d") % (filename, filetype, numspline, first,
                                 last, rayslayer, lenslayer,
                                 int(exportdummy), int(usesolids),
                                 raypattern, numrays, wave, field,
                                 int(deletevignetted), dummythick,
                                 int(split), int(scatter),
                                 int(usepol), config)
        return self.req(cmd)
    ...
    def GetAperture(self, surf):
        ...
        (_type, _min, _max, _xdecenter, _ydecenter,
         _aperturefilename) = response.split(",")
        return (int(_type), float(_min), float(_max),
                float(_xdecenter), float(_ydecenter), _aperturefilename)
    ...
    def GetFieldsConfig(self):
        ...
        (_type, _number, _max_x_field, _max_y_field,
         _normalization_method) = response.split(",")
        return (int(_type), int(_number), float(_max_x_field),
                float(_max_y_field), int(_normalization_method))

    def GetField(self, n):
        assert(n > 0)  # 0 gets configuration - see GetFieldsConfig
        response = self.req("GetField,%d" % n)
        (xfield, yfield, weight, vdx, vdy, vcx, vcy,
         van) = (float(n) for n in response.split(","))
    ...
    def GetMulticon(self, config, row):
        ...
        vals = response.split(",")
        if len(vals) > 6:
            (_value, _num_config, _num_row, _status, _pickuprow, _pickupconfig,
             _scale, _offset) = vals
            _scale = float(_scale)
            _offset = float(_offset)
        else:
            (_value, _num_config, _num_row, _status, _pickuprow,
             _pickupconfig) = vals
            _scale = _offset = None
        # Status :
        #  0 : fixed
    ...
    def GetRefresh(self, untraceable_allowed=False):
        # Copies lens back from Lens Data Editor, then performs
        # equivalent of GetUpdate
    ...
    def GetSystem(self):
        ...
        (numsurfs, unitcode, stopsurf, nonaxialflag, rayaimingtype,
         adjust_index, temp, pressure, globalrefsurf) = response.split(",")
        # Zemax manual specifies 9 parameters but we only observe 8
        # (need_save missing)
        # unitcode : {0:mm, 1:cm, 2:in, 3:M}
        return (int(numsurfs), int(unitcode), int(stopsurf),
    ...
    def GetTextFile(self, path, type, settingspath, flag, **kwargs):
        if flag == 2:
            # Zemax creates a dialog box for the user to close. We
            # need to wait for that.
            kwargs["timeout"] = 2**28
        response = self.req("GetTextFile,\"%s\",%s,\"%s\",%d" % (
            path, type, settingspath, flag), **kwargs)
        return response

    def GetTextFileString(self, type, settingspath=None, flag=0,
                          timeout=120000):
    ...
    @contextmanager
    def GetTextFileObject(self, type, settingspath=None, flag=0,
                          timeout=120000):
        """Returns the output of GetTextFile as a file-like context manager.

        This is useful when we want temporary file handling, but we
        want to handle the file one line at a time."""
        if settingspath is None:
            settingspath = ""
        elif not flag:
            # `settingspath` has been set.  For it to take effect,
            # ensure `flag` is 1, otherwise Zemax default will be used.
            flag = 1
        ...
        def acquire(resultsf):
            return self.GetTextFile(resultsf, type, settingspath,
                                    flag, timeout=timeout)

        # If the timeout expires here, Zemax will not have released
        # the file for writing.  This means the tmpfile context
        # manager will fail.  The work-around is to specify a longer
        # timeout value on the request.

        # TODO: provide an option to prevent DDE request timeouts
        # triggering before a resource (eg. temporary file) is released.
    ...
    def GetTrace(self, wave, mode, surf, h, p):
        ...
        (status, vigcode, x, y, z, l, m, n, l2, m2, n2,
        ...
        def float_array(a):
            return array([float(e) for e in a])
    ...
    def GetTraceDirect(self, wave, mode, startsurf, stopsurf, origin, cosines):
        ...
        cmd = ("GetTraceDirect,%d,%d,%d,%d,%.20E,%.20E,%.20E,%.20E,"
               "%.20E,%.20E") % (wave, mode, startsurf, stopsurf,
    ...
    def LoadMerit(self, filename):
        ...
        # Manual states: If the merit function value is 9.00e+009, the
        # merit function cannot be evaluated
    ...
    def NSCTrace(self, surf=1, source=0, split=0, scatter=0,
                 usepolar=1, ignore_errors=0, no_random_seed=0,
                 save=0, savefilename="", filter="", zrd_format=2,
                 timeout=600000):
        args = (surf, source, split, scatter, usepolar, ignore_errors,
                no_random_seed, save, savefilename, filter, zrd_format)
    ...
    def PushLens(self, flag=0):
        ...
        if error == -999:
            raise Exception(
                "Lens cannot be pushed into Lens Data Editor. This operation "
                "must be enabled in the Zemax preferences.")
    ...
    def SaveDetector(self, surf, obj, filename, timeout=60000):
        error = self.req("SaveDetector,%d,%d,%s" % (surf, obj, filename),
                         timeout=timeout)
        return int(error)
    ...
    def SetAperture(self, surf, _type, _min=0.0, _max=0.0, xdecenter=0.0,
                    ydecenter=0.0, aperturefile=""):
        ...
        type :
         ...
         10: Floating
        """
        response = self.req("SetAperture,%d,%d,%.20E,%.20E,%.20E,%.20E,%s" %
                            (surf, _type, _min, _max, xdecenter, ydecenter,
                             aperturefile))
        ...
        try:
            ...
            aperturefile = None
        return (int(_type), float(_min), float(_max),
                float(xdecenter), float(ydecenter), aperturefile)
    ...
    def SetExtra(self, surf, column, value):
        newvalue = self.req("SetExtra,%d,%d,%s" % (surf, column,
                                                   self._str(value)))
        return newvalue
    ...
    def SetFieldsConfig(self, _type, _number, _normalization):
        ...
        #  0 : radial
        #  1 : rectangular
        response = self.req("SetField,0,%d,%d,%d" % (_type, _number,
                                                     _normalization))
        return response

    def SetField(self, n, xf, yf, wgt=1.0, vdx=0.0, vdy=0.0, vcx=0.0,
                 vcy=0.0, van=0.0):
        assert(n > 0)  # 0 gets configuration
        response = self.req("SetField,%d,%.20E,%.20E,%.20E,%.20E,%.20E,%.20E,"
                            "%.20E,%.20E" % (n, xf, yf, wgt,
                                             vdx, vdy, vcx, vcy, van))
        (xfield, yfield, weight, vdx, vdy, vcx, vcy,
         van) = (float(n) for n in response.split(","))
    ...
    def SetMulticon(self, config, row, value, status=0, pickuprow=1,
                    pickupconfig=1, scale=1.0, offset=0.0):
        assert(config)  # use GetMulticonConfig to handle this
        cmd = "SetMulticon,%d,%d,%s,%d,%d,%d,%.20E,%.20E" % (
            config, row, self._str(value), status, pickuprow, pickupconfig,
            scale, offset)
        response = self.req(cmd)
        items = response.split(",")
        # _value, _num_config, _num_row, _status = items[:4]
        (_value, _num_config, _num_row, _status, _pickuprow,
         _pickupconfig) = items[:6]
        # Status :
        #  0 : fixed
        #  1 : variable
        #  2 : pickup
    ...
    def SetNSCObjectData(self, surf, obj, code, value):
        cmd = "SetNSCObjectData,%d,%d,%d,%s" % (surf, obj, code,
                                                self._str(value))
        return self.req(cmd)

    def SetNSCParameter(self, surf, obj, param, value):
        cmd = "SetNSCParameter,%d,%d,%d,%s" % (surf, obj, param,
                                               self._str(value))
        return self.req(cmd)

    def SetNSCPosition(self, surf, obj, code, value):
        cmd = "SetNSCPosition,%d,%d,%d,%s" % (surf, obj, code,
                                              self._str(value))
        return self.req(cmd)

    def SetNSCProperty(self, surf, obj, code, face, value):
        cmd = "SetNSCProperty,%d,%d,%d,%d,%s" % (surf, obj, code, face,
                                                 self._str(value))
        return self.req(cmd)
    ...
    def SetSolve(self, surf, code, *args):
        # Sending undefined solvetype codes does not result in an
        # error from Zemax
        cmd = ("SetSolve,%d,%d," % (surf, code) +
               ",".join(self._str(a) for a in args))
        response = self.req(cmd)
        return response

    def SetSurfaceData(self, surf, code, data):
        cmd = "SetSurfaceData,%d,%d,%s" % (surf, code,
                                           self._str(data))
        response = self.req(cmd)
    ...
    def SetSurfaceParameter(self, surf, code, data):
        cmd = "SetSurfaceParameter,%d,%d,%s" % (surf, code,
                                                self._str(data))
        response = self.req(cmd)
        # response is usually the data value echoed back
        return response

    def SetSystem(self, unitcode, stopsurf, rayaimingtype, temp,
                  pressure, globalrefsurf):
        # argument #4 "useenvdata" is ignored, according to manual
        response = self.req("SetSystem,%d,%d,%d,0,%.20E,%.20E,%d" % (
            unitcode, stopsurf, rayaimingtype, temp, pressure, globalrefsurf))
        (numsurfs, unitcode, stopsurf, nonaxialflag, rayaimingtype,
         adjust_index, temp, pressure, globalrefsurf) = response.split(",")
        # unitcode : {0:mm, 1:cm, 2:in, 3:m}
        return (int(numsurfs), int(unitcode), int(stopsurf),
                int(nonaxialflag), int(rayaimingtype),
                int(adjust_index), float(temp), float(pressure),
                int(globalrefsurf))
    ...
    def SetSystemAper(self, _type, _stopsurf, _aperture_value):
        response = self.req("SetSystemAper,%d,%d,%.20E" %
                            (_type, _stopsurf, _aperture_value))
        _type, _stopsurf, _aperture_value = response.split(",")
        # type :
        #  0 : entrance pupil diameter
        #  1 : image space f/#
