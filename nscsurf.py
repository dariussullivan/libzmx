...
# It's hard to handle Nonsequential object references as gracefully as
# sequential surface references because there is no Set/Get/FindLabel
# method for them.  Still, there's probably a better approach than the
# one taken here.
...
class NonSequentialComponent(UnknownSurface):
    ...
    def set_obj_position(self, slot, offset_x=None, offset_y=None,
                         offset_z=None, rotate_x=None, rotate_y=None,
                         rotate_z=None):
    ...
    def set_obj_ignored(self, slot, ignore, on_launch_only=False):
        ...
        if ignore:
            ...
            if on_launch_only:
                status = 2
        return int(float(self.conn.SetNSCProperty(self.get_surf_num(),
                                                  slot, 16, 0,
                                                  status)))
    ...
    def insert_std_surf(self, slot, comment=None, radius=None,
                        conic=None, max_aper=None):
    ...
    def insert_toroidal_surf(self, slot, comment=None, radius=None,
                             radius_of_rotation=None, hwidth_x=None,
                             hwidth_y=None):
    ...
    def insert_drect(self, slot, comment=None, n_pixels_x=None,
                     n_pixels_y=None):
    ...
    def insert_two_angle_src(self, slot, half_widths, half_angles,
    ...
    def insert_std_lens(self, slot, comment=None, radii=None,
                        semidias=None, thickness=None):
    ...
    def insert_lenslet_array(self, slot, comment=None, thickness=None,
                             groove_freq=None, order=None,
                             diffract_face=None):
...
def get_detector_data(conn, settingspath=None):
    with conn.GetTextFileObject("Dvr", settingspath) as f:
        ...
        while not _dvr_cols_re.match(next(f)):
            pass
        # read detector data
        for i in range(sz[1]):
            row = next(f).split('\t')
